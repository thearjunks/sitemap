...
export function StatusDashboard() {
  ...
  return <main className="standalone-shell">
    ...
    <div className="status-top"><div><a href="/" className="back-link">← STC dashboard</a><h1>URL Status Dashboard</h1><p className="subhead">Add URLs from any domain and monitor their current HTTP status.</p></div><button className="btn primary" disabled={Boolean(checking)} onClick={() => check()}>{checking ? `Checking ${checking.done} / ${checking.total}` : "⟳ Check all now"}</button></div>
    <section className="panel add-panel"><div><div className="panel-title">Add URLs</div><div className="panel-meta">Paste one URL per line, or separate with commas.</div></div><div className="add-grid"><textarea aria-label="URLs to add" placeholder={'https://example.com/\nhttps://example.com/contact'} value={input} onChange={(event) => setInput(event.target.value)} /><button className="btn primary" onClick={add}>＋ Add URLs</button></div>{message && <div className="status-message">{message}</div>}</section>
