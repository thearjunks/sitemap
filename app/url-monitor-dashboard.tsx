  ...
  return (
    <div className="app-shell">
      ...
      {drawer && <div className="drawer-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) setDrawer(null); }}><aside className="drawer" role="dialog" aria-modal="true">
        ...
        {drawer === "add" && <><div className="field"><label>Website URL</label><input autoFocus placeholder="https://www.example.com/page" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} /></div><div className="field"><label>Label (optional)</label><input placeholder="Pricing page" value={labelInput} onChange={(e) => setLabelInput(e.target.value)} /></div><div className="field"><label>Group</label><input value={groupInput} onChange={(e) => setGroupInput(e.target.value)} /></div><div className="drawer-actions"><button className="btn" onClick={() => setDrawer(null)}>Cancel</button><button className="btn primary" onClick={addOne}>Add URL</button></div></>}
        {drawer === "bulk" && <><div className="field"><label>Paste URLs</label><textarea autoFocus placeholder={'https://example.com/page-one\nhttps://example.com/page-two'} value={bulkInput} onChange={(e) => setBulkInput(e.target.value)} /><div className="field-help">One URL per line, or separate using commas. Duplicate and invalid URLs are skipped.</div></div><div className="field"><label>Group for this import</label><input value={groupInput} onChange={(e) => setGroupInput(e.target.value)} /></div><div className="drawer-actions"><button className="btn" onClick={() => setDrawer(null)}>Cancel</button><button className="btn primary" onClick={addBulk}>Import URLs</button></div></>}
